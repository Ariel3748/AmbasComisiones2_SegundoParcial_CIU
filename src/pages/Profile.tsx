...
import { ChatRightText, Heart, Trash3 } from 'react-bootstrap-icons';
      ...
      {loading ? (
        ...
        <div className="d-flex flex-column gap-3">
          {misPosts.map((post) => {
            ...
            const liked = userContext !== null && post.likedBy?.some((id) => id === userContext._id);
            return (
                    ...
                    <div className="d-flex align-items-center gap-3">
                      <button
                        onClick={async () => {
                          if (!userContext) return;
                          try {
                            const res = await apiService.toggleLike(post._id, userContext._id);
                            setMisPosts((prev) =>
                              prev.map((p) =>
                                p._id === post._id ? { ...p, votes: res.votes, likedBy: res.liked ? [...p.likedBy, userContext._id] : p.likedBy.filter((id) => id !== userContext._id) } : p
                              )
                            );
                          } catch {
                            // silent
                        ...
                        style={{ color: liked ? '#dc3545' : 'var(--reddit-muted)', fontSize: '14px' }}
                      >
                        <Heart size={16} />
                      </button>
                      <span>{post.votes || 0}</span>
                      <span><ChatRightText size={16}/> {commentCount} comentarios visibles</span>
                    </div>
